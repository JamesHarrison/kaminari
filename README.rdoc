...
This fork is a nasty bunch of hacks added to Kaminari to remove the requirement to have an overall total count for a paginated collection since performance for MongoDB's count operand is abysmal (requires a table scan).

It should probably never be used anywhere, but is being used as a temporary hack.
